...
uint32_t displaySeqMenu(char c,sequencer* seqN,uint8_t* exit_loop) { 					// displaySeqMenu() lets the user edit specific settings of the sequencer passed in parameter.
	...
	printf("\t\t\t'q' to quit.\n");
	fflush(stdout);
	while (!((c2=='q'||c2=='m'||c2=='d'||c2=='r'||c2=='s'||c2=='f'||c2=='g') && c==0 )  // if Seq is passed in parameter to the function, characters typed can be 'm', 'd', 'r', 's', 'f',
		...
		if (c2=='q') *exit_loop=1;														// If character typed is 'q' : Exits loop.
			...
			if (c2=='m') {																// If character typed is 'm' : Edits the mask.
				...
				if (!frozen) {															//   If Seq is not frozen.
					...
					errors+=CHAN_setByte(IO24_NUM,0);									//    Writes 0 at current address : To synchronize Seq with any step of Seq itself (In mode 0 only Seq is enforced).	 
			...
			else if (c2=='g') {															// If character typed is 'f' : Freezes/releases sequencer.
				errors+=CHAN_addr(IO24_NUM,0x1CB);										//  Sets current address at address of SEQS_FREEZE_MASK.														
				if (!frozen) {															//   If Seq is not frozen.
					errors+=CHAN_setByte(IO24_NUM,1);									//    Writes 1 at current address to define Seq as sequencer to freeze.
					CHAN_command(IO24_NUM,2);											//    Executes IO24 command 0x02 named FREEZE : The sequencers defined by SEQS_FREEZE_MASK are frozen.
					...
					errors+=CHAN_setByte(IO24_NUM,seqN->mask);							//    Writes seqN.mask at current address to force bits 7, 6, 5 and 4 to 1 on OutputB.
					frozen=1;
					printf("Sequencer Seq is frozen.\n");
					fflush(stdout);
				}
...
int main (int argc,char *argv[ ]) {
	...
	sequencer seq1={0xF0,0x151};									// The sructure storing specific settings of sequencer Seq1.
	...
	************************************************************************************************************************************************************************************************************************************************/
	...
	printf("with SEQ_ASTEP_INC_TIME=29 and Tseq=5ms ");
	printf("then Ts=30*5ms=150ms\n\n");
	displayMainMenu();
