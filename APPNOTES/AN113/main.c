 /*******************************************************************
 *                                                                  * 
 * Chantilly 0x10 - IO24 : Source code for application note AN113   *
 *                                                     			    *
 ****************************************************************************************************************************************************************************************************************************************************
 * CIRCUIT DESCRIPTION :																																					                                                                        *
 ...
 * As described in file chantilly_0x10_AN113_pcb2rev1.PDF, a bar led is wired on OutputA and OutputB.																																				*
 * 																																																													*
 *  																																																												* 
 * PROGRAM DESCRIPTION : Test using IO24 direct core access mode with (immediate inputs and) sequenced outputs in mode 3.                                                                    		                                               	*
 * Due to mode 3, sequencers Seq, Seq1 and Seq2 play a common sequence. 																																											*
 * SEQ_MASK, SEQ1_MASk and SEQ2_mask initial values make Seq outputed on bit0, bit1, bit2, bit3, bit6 and bit7 of OutputA, Seq1 outputed on bit4 and bit5 of outputA and Seq2 outputed on bit4, bit5, bit6 and bit7 of OutputB.						*
 * 																																																													*
 * 																																																													*
 *  												                                                   																																				*		
 *                                       									                  													                                                   													*
...
{																		// SEQ_WRAP is a common setting to the three sequencers Seq, Seq1 and Seq2 in mode3.
	uint8_t wrap;
	uint32_t wrapAddr;
}
sequencers;                                                             
...
sequencer;																// Structure storing specific settings to each sequencer.
*/
...
/*uint8_t displaySeqMenu(char c,sequencer* seqN,uint32_t* errors,uint8_t* initC) { 			// displaySeqMenu() lets the user edit specific settings of the sequencer passed in sequencer.
	uint8_t c2='\0';																		// Using mode 3 implies Seq, Seq1 or Seq2 can be passed in parameter.
	uint8_t exit_loop=0;
	if (c!='0') printf("\nSequencer Seq%c menu\n",c);
	else printf("\nSequencer Seq menu\n");
	printf("\tType\t:\t'm' to edit mask.\n\t\t\t's' to invert step.\n\t\t\t'd' to change steps delay.\n\t\t\t'r' to return.\n\t\t\t'q' to quit.\n");
	fflush(stdout);
	while (c2!='q'&&c2!='m'&&c2!='d'&&c2!='r'&&c2!='s') {
		getch_start();																		//
		c2=getch_try();																		//
	...
	if (c2) {
		if (c2=='q') exit_loop=1;															// If character typed is 'q' : Exits loop.
			...
			if (c2=='m') {																	// If character typed is 'm' : Edits the mask.
				...
				*errors+=CHAN_addr(IO24_NUM,seqN->maskAddr);								// Switch seqN parameter passed to the function, sets current address at address of SEQ_MASK, SEQ1_MASK or SEQ2_MASK.
				...
				*errors+=CHAN_addr4(IO24_NUM,seqN->stepAddr4);								//  Switch seqN parameter passed to the function, sets current address at address of SEQ_STEP, SEQ1_STEP or SEQ2_STEP into IO24.
				*errors+=CHAN_setByte(IO24_NUM,seqN->step);									//  Writes seqN->step at current address.
				if (c!='0') { 
					...
					else if (seqN->step==2) printf("sequencer Seq decreases\n");			//  Switch value of step displays the new sense of the sequencer Seq1 or the sequencer Seq2 passed to the function.
				}
				else {
					if (seqN->step==0) printf("sequencer Seq%c does nothing\n",c);			//
					else if (seqN->step==1) printf("sequencer Seq%c increases\n",c);		//
					else if (seqN->step==2) printf("sequencer Seq%c decreases\n",c);		//  Switch value of step displays the new sense of the sequencer Seq passed to the function.
				}
				*initC=1;																	// Used to restart the sequence when sense has changed. 			
			}
				...
				if (c!='0') {																//  If the sequencer passed to the function is not Seq.
					printf("Seq%c steps delay Ts%c is such as",c,c);						//   Displays SEQ1_ASTEP_INC_TIME and current steps delay of the sequencer Seq1.
					printf(" Ts%c=Tseq*(SEQ%c_ASTEP_INC_TIME+1)",c,c);						//   or displays SEQ2_ASTEP_INC_TIME and current steps delay of the sequencer Seq2
					printf(" with Tseq=5ms and ");											//
					printf("SEQ%c_ASTEP_INC_TIME=%d\n",c,seqN->astepIncTime);				//
					printf("Enter new value for SEQ%c_ASTEP_INC_TIME=",c);					//   Asks a new vale for SEQ1_ASTEP_INC_TIME.
				}                                   
				else {								                                        //  Else 										
				...
				scanf("%3d",(unsigned int*)&seqN->astepIncTime);							// Stores the new value of SEQ_ASTEP_INC_TIME or SEQ1_ASTEP_INC_TIME into variable seqN.astepIncTime.
					
				*errors+=CHAN_addr(IO24_NUM,seqN->astepIncTimeAddr);						// Switch seqN parameter passed to the function, sets current address at address of SEQ_ASTEP_INC_TIME, SEQ1_ASTEP_INC_TIME or SEQ2_ASTEP_INC_TIME.
				*errors+=CHAN_setByte(IO24_NUM,seqN->astepIncTime);							// Writes seqN->astepIncTime at current address.
				if (c!='0') {																//  If the sequencer passed to the function is not Seq.
					...
					printf("(SEQ%c_ASTEP_INC_TIME+1)=%dms\n",c,(seqN->astepIncTime+1)*5);	//   Displays new steps delay of Seq1 or new steps delay of Seq2.
				}
				else {																		//  Else
			...
			if (c2!='r') return displaySeqMenu(c,seqN,errors,initC);						// If charactere typed is not 'r' displays recursively menu of the sequencer passed to the function.
			else printf("return\n");														// Else 
...
uint32_t displaySeqMenu(char c,sequencer* seqN,uint8_t* exit_loop) { 						// displaySeqMenu() lets the user edit specific settings of the sequencer passed in parameterr.
	uint8_t c2='\0';																		// Using mode 2 implies Seq, Seq1 or Seq2 can be passed in parameter.
	uint32_t errors=0;
	uint8_t buf[8];
	if (c=='0') c=0;
	printf("\nSequencer Seq%c menu\n",c);
	...
	if (c2) {
		if (c2=='q') *exit_loop=1;															// If character typed is 'q' : Exits loop.
			...
			if (c2=='m') {																	// If character typed is 'm' : Edits the mask.
				...
				printf("Enter new value for SEQ%c_MASK = 0x",c);
				scanf("%2x",(unsigned int*)&seqN->mask);	
				errors+=CHAN_addr(IO24_NUM,seqN->maskAddr);									//  Switch the parameter seqN passed to the function, sets current address at address of SEQ_MASK, SEQ1_MASK or SEQ2_MASK.
				errors+=CHAN_setByte(IO24_NUM,seqN->mask);									//  Writes seqN.mask at current address.	
			}
			else if (c2=='s') {																// If character typed is 's' : Edits the sense.
				if (seqN->step==1) seqN->step=2;
				...
				errors+=CHAN_addr4(IO24_NUM,seqN->stepAddr4);								//   Switch the parameter seqN passed to the function, sets current address at address of SEQ_STEP, SEQ1_STEP or SEQ2_STEP.
				errors+=CHAN_setByte(IO24_NUM,seqN->step);									//   Writes seqN.step at current address.
				if (seqN->step==0) printf("sequencer Seq%c does nothing\n",c);				//
				else if (seqN->step==1) printf("sequencer Seq%c increases\n",c);			//
				else if (seqN->step==2) printf("sequencer Seq%c decreases\n",c);			//   Switch the parameter seqN passed to the function, displays the new sense of the sequencer Seq, Seq1 or Seq2. 
			}
				...
				printf("Seq%c steps delay Ts%c is such as",c,c);							//   Displays SEQ_ASTEP_INC_TIME, SEQ1_ASTEP_INC_TIME or SEQ2_ASTEP_INC_TIME and current steps delay of the sequencer Seq, Seq1 or Seq2.
				printf(" Ts%c=Tseqs*(SEQ%c_ASTEP_INC_TIME+1)",c,c);							//
				printf(" with Tseqs=5ms and ");												//
				printf("SEQ%c_ASTEP_INC_TIME=%d\n",c,seqN->astepIncTime);					//
				printf("Enter new value for SEQ%c_ASTEP_INC_TIME=",c);						//   Asks a new value for SEQ_ASTEP_INC_TIME, SEQ1_ASTEP_INC_TIME or SEQ2_ASTEP_INC_TIME.
				scanf("%3d",(unsigned int*)&seqN->astepIncTime);							//   Stores the new value of SEQ_ASTEP_INC_TIME, SEQ1_ASTEP_INC_TIME or SEQ2_ASTEP_INC_TIME into variable seqN.astepIncTime.
				CHAN_command(IO24_NUM,2);													//   Executes IO24 command 0x02 named FREEZE. Freezes the sequencers defined by SEQS_FREEZE_MASK.
				errors+=CHAN_addr(IO24_NUM,seqN->astepIncTimeAddr);							//   Sets current address at address of SEQ_ASTEP_INC_TIME, SEQ1_ASTEP_INC_TIME or SEQ2_ASTEP_INC_TIME.
				errors+=CHAN_setByte(IO24_NUM,seqN->astepIncTime);							//   Writes seqN.astepIncTime at current address.
				printf("Seq%c steps delay Ts%c ",c,c);										//
				printf("is now set such as Ts%c=Tseqs*",c);									//
				printf("(SEQ%c_ASTEP_INC_TIME+1)=%dms\n",c,(seqN->astepIncTime+1)*5);		//   Switch the sequencer passed to the function, displays new steps delay of Seq, Seq1 or Seq2.
				errors+=CHAN_addr(IO24_NUM,0x14B);											//    Sets current address at address of SEQS_UNFREEZE_POSITION into IO24.
				...
				if (!seqN->frozen){
					...
					printf("\t\t\t\t");														//
					printf("Tf%c=%dms\n",c,buf[0]*(seqN->astepIncTime+1)*5);				//    Switch the sequencer passed to the function, displays SEQ_FREEZE_COUNT, SEQ1_FREEZE_COUNT or SEQ2_FREEZE_COUNT and frozen time values.
					fflush(stdout);
				}
			}
			else if (c2=='g') {																// If character typed is 'g' : Freezes & forces/releases sequencer.
				...
				if (!seqN->frozen){
					CHAN_command(IO24_NUM,2);												//    Executes IO24 command 0x02 named FREEZE : The sequencers defined by SEQS_FREEZE_MASK are frozen.
					errors+=CHAN_addr(IO24_NUM,seqN->savedCursorAddr);						//    Sets current address at address of SEQ_SAVED_CURSOR, SEQ1_SAVED_CURSOR or SEQ2_SAVED_CURSOR.
					errors+=CHAN_setByte(IO24_NUM,seqN->mask);								//    Writes seqN.mask at current address to force all bits defined by seqN.mask to 1 on OutputA for Seq and Seq1 or OutputB for Seq2. 
					seqN->frozen=1;
					printf("Sequencer Seq%c is frozen.\n",c);								//
				}
				else {
				...
				CHAN_command(IO24_NUM,2);													//   Executes IO24 command 0x02 named FREEZE. Freezes the sequencers defined by SEQS_FREEZE_MASK.
				errors+=CHAN_addr(IO24_NUM,seqN->cursorAddr);								//   Sets current address at address of SEQ_CURSOR, SEQ1_CURSOR or SEQ2_CURSOR.
				errors+=CHAN_setByte(IO24_NUM,seqN->cursor);								//   Writes new value of SEQ_CURSOR, SEQ1_CURSOR or SEQ2_CURSOR switch parameter of the function.
				CHAN_command(IO24_NUM,4);													//   Executes IO24 command 0x04 named RELEASE : The sequencers defined by SEQS_FREEZE_MASK are released and synchronized.
...
uint32_t pauseResumeSteps(sequencer* seqA, sequencer* seqB, sequencer* seqC) {              // pauseResumeSteps() pauses/resumes the three sequencers passed in parameters.
	uint32_t errors=0;
	static uint8_t stepA=0,stepB=0,stepC=0;													 
	if (!stepA) {																			// If Seq increases or decreases.
		...
		stepC=seqC->step;																	// 	Saves value of seqC.step in stepC.
		seqC->step=0;																		//  Sets seqC.step to 0.
	}
	else {																					// Else	
		printf("running\n");																//
		...
		seqC->step=stepC;																	//  Sets seqC.step to saved value of stepC.
		stepC=0;																			//  Resets stepC.
	}
	...
	errors+=CHAN_addr4(IO24_NUM,seqB->stepAddr4);											// Sets current address at address of SEQ1__STEP.
	errors+=CHAN_setByte(IO24_NUM,seqB->step);												// Writes value of seqB.step at current address.
	errors+=CHAN_addr4(IO24_NUM,seqC->stepAddr4);											// Sets current address at address of SEQ2__STEP.
	errors+=CHAN_setByte(IO24_NUM,seqC->step);												// Writes value of seqC.step at current address.
	displayMainMenu();
	return errors;
}
...
int main (int argc,char *argv[ ]) {
	...
	sequencer seq={0xCF,0x150,1,0x50,0x04,0x141,29,0x144,2,0x145,0,0x00,0,0x1C8,0,1};	// The sructure storing specific settings of sequencer Seq.
	//seq.mask=0xCF;									
	//seq.maskAddr=0x150;								
	//seq.step=1;										
	//seq.stepAddr4=0x50;								
	...
	sequencer seq1={0x30,0x151,1,0x70,0x04,0x1C1,29,0x1C2,2,0x1C3,0,0x01,0,0x1C9,0,2};	// The sructure storing specific settings of sequencer Seq1.
	//seq1.mask=0x30;								
	//seq1.maskAddr=0x151;							
	//seq1.step=1;									
	//seq1.stepAddr4=0x70;							
	...
	//seq1.astepIncTime=29;
	//seq1.astepIncTimeAddr=0x1C2;
	//seq1.unfreezeResync=2;
	//seq1.unfreezeResyncAddr=0x1C3;
	//seq1.savedCursor=0;
	//seq1.savedCursorAddr=0x01;
	//seq1.freezeCount=0;
	//seq1.freezeCountAddr=0x1C9;		
	//seq1.frozen=0;
	//seq1.freezeMask=2;
	...
	sequencers seqs={1,0x146};								// The structure storing common settings of the enforced sequencers Seq, Seq1 and Seq2.	
	//seqs.wrap=1;											// SEQ_WRAP is as common setting of the three sequencers.
	//seqs.wrapAddr=0x146;					
	...
	************************************************************************************************************************************************************************************************************************************************/
	...
	errors+=CHAN_setByte(IO24_NUM,seq.mask);		// Writes seq.mask(=0xCF) at current address 0x150 into IO24. Used to set SEQ_MASK=0xCF
	errors+=CHAN_setByte(IO24_NUM,seq1.mask);		// Writes seq1.mask(=0x30) at current address 0x151 into IO24. used to set SEQ1_MASK=0x30
	...
	// Since mode of sequencer is 3, Seq and Seq1 play on OutputA such as OutputA=data[SEQ_CURSOR]&SEQ_MASK|data[SEQ1_CURSOR]&SEQ1_MASK|OUTPUT_A with data[X]:value stored at address X and 0x04≤X≤0xFF.
	//								 Seq2 plays on OutputB such as OutputB=data[SEQ2_CURSOR]&SEQ2_MASK|OUTPUT_B.
	// With above settings Seq plays on OutputA bit7, bit6, bit3, bit2, bit1 and bit0, Seq1 plays on OutputA bit5 and bit4 and Seq2 plays on OutputB bit7, bit6, bit5 and bit4. 
	...
	// Using mode 3 implies that a common sequence for Seq and Seq1 on OutputA and for seq2 on OutputB must be stored between included addresses 0x04 and 0xFF in RAM.
	...
	myTXBuffer[2]=0x04;								// SEQ_START=0x04   		RAM address where (common) Seq, Seq1 and Seq2 sequence begins.
	myTXBuffer[3]=0x07;   							// SEQ_END=0x08   		 	RAM address where (common) Seq, Seq1 and Seq2 sequence ends. In mode 3, 0x04≤SEQ_START≤SEQ_END≤0xFF.
	...
	myTXBuffer[7]=3;								// SEQ_MODE=3   		 	the three sequencers Seq, Seq1 and Seq2 are independants and play a common sequence. That means sequencers Seq, Seq1 and Seq2 settings are required.
	...
	// All others sequencer Seq settings can be edited when sequenced outputs mode is running but it is not advisable.
	
	
	/** SETS UP SEQUENCER Seq1 SETTINGS */
													// Presets via myTXBuffer :
	myTXBuffer[0]=seq1.step;   						// SEQ1_STEP=seq1.step=1   	sequencer Seq1 increases.
	myTXBuffer[1]=seq1.cursor;						// SEQ1_CURSOR=0x04   	   	current RAM address of Seq1 sequence (permanently overwritten but as Seq1 increase must be initialized such as SEQ1_CURSOR≤SEQ_END).
	myTXBuffer[2]=seq1.astepIncTime;				// SEQ1_ASTEP_INC_TIME     	used to change delay Ts1 between steps of Seq1 sequence such as Ts1=(SEQ1_ASTEP_INC_TIME+1)*Tseq=31*5.014ms=155.434 ms. => fs1=6.43Hz. 
	myTXBuffer[3]=seq1.unfreezeResync;				// SEQ1_UNFREEZE_RESYNC=2   Seq1 is synchronized with Seq2 when released : Seq2 is time reference for Seq1.
	errors+=CHAN_addr4(IO24_NUM,0x70);				// sets current address at 0x70*4=0x1C0 into IO24.
	errors+=CHAN_setBytes(IO24_NUM,4,myTXBuffer);	// writes sequencer Seq1 settings (stored in myTXBuffer) at current address 0x1C0 in RAM.
	// All sequencer Seq1 settings can be edited when sequenced outputs mode is running but it is not advisable.

	
	/** SETS UP SEQUENCER Seq2 SETTINGS */
		
	myTXBuffer[0]=seq2.step;   						// SEQ2_STEP=seq2.step=1 	sequencer Seq2 increases.
	myTXBuffer[1]=seq2.cursor;						// SEQ2_CURSOR=0x04 		current RAM address of Seq2 sequence. (permanently overwritten but as Seq1 increase must be initialized such as SEQ2_CURSOR≤SEQ_END).
	...
	myTXBuffer[3]=seq2.unfreezeResync;				// SEQ2_UNFREEZE_RESYNC=0   Seq2 is synchronized with Seq when released : Seq is time reference for Seq2.
	errors+=CHAN_addr4(IO24_NUM,0x71);				// Sets current address at 0x71*4=0x1C4 into IO24.
	errors+=CHAN_setBytes(IO24_NUM,4,myTXBuffer);	// Writes sequencer Seq2 settings (stored in myTXBuffer) at current address 0x1C4 in RAM.
	...
	************************************************************************************************************************************************************************************************************************************************/
	...
	printf("\nIO24 sequenced outputs mode 3 is started : \n");
	printf("Sequencers Seq, Seq1 and Seq2 are running with respective steps delays Ts, Ts1 and Ts2 set such as ");
	printf("Ts=Tseq*(SEQ_ASTEP_INC_TIME+1), Ts1=Tseq*(SEQ1_ASTEP_INC_TIME+1) and Ts2=Tseq*(SEQ2_ASTEP_INC_TIME+1) ");
	printf("with SEQ_ASTEP_INC_TIME=SEQ1_ASTEP_INC_TIME=SEQ2_ASTEP_INC_TIME=30 and Tseq=5ms ");
	printf("then Ts=Ts1=Ts2=31*5ms=155ms\n\n");
	//printf("\tType\t:\t'0' to edit Sequencer Seq.\n\t\t\t'1' to edit Sequencer Seq1.\n\t\t\t'2' to edit Sequencer Seq2.\n\t\t\t'w' to invert wrap.\n\t\t\t'p' to pause/resume.\n\t\t\t'q' to exit.\n");
	displayMainMenu();
	...
	while (!exit_loop) {														// Entering loop
		...
		if (c) {                                                    			//  if something is typed :
			...
			else if (c=='p') errors+=pauseResumeSteps(&seq,&seq1,&seq2);		//	 if 'p' is typed pauses/resumes the three sequencers. pauseSteps() takes &seq,&seq1,&seq2 as parameters because SEQ_STEP, SEQ1_STEP, SEQ2_STEP are independent.
			else {														
