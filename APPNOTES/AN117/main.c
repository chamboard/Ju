 /*******************************************************************
 *                                                                  * 
 * Chantilly 0x10 - IO24 : Source code for application note AN117   *
 *                                                     			    *
 ****************************************************************************************************************************************************************************************************************************************************
 * CIRCUIT DESCRIPTION :																																					                                                                        *
 ...
 * As described in file chantilly_0x10_AN117_pcb2rev1.PDF, three 4 wires stepper motors are wired on OutputA and MSBs of OutputB.																													*
 * 																																																													*
 *  																																																												* 
 ...
 * SEQ_MASK, SEQ1_MASk and SEQ2_mask values make Seq outputed on bit0, bit1, bit2, bit3, bit6 and bit7 of OutputA, Seq1 outputed on bit4 and bit5 of outputA and Seq2 outputed on bit4, bit5, bit6 and bit7 of OutputB.								*
 * 																																																													*
 * 																																																													*
 *  												                                                   																																				*		
 *                                       									                  													                                                   													*
...
void pauseResumeSteps(sequencer* seqA, sequencer* seqB, sequencer* seqC, uint32_t* errors) {// pauseResumeSteps() pauses/resumes the three sequencers passed in parameters.
	static uint8_t stepA=0,stepB=0,stepC;											 
	...
	if (!stepC) {																			// If Seq2 increases or decreases. 
		printf("%s paused\n",seqC->name);													//  
		stepC=seqC->presetStep;																//  Saves value of seqB.presetStep in stepB.
		seqC->presetStep=0;																	//  Sets seqB.presetStep to 0.
	}
	else {																					// Else
		printf("%s running\n",seqB->name);
		seqB->presetStep=stepB;																//	Sets seqB.presetStep to saved value of stepB.
	...
	*errors+=CHAN_addr(IO24_NUM,seqC->presetStepAddr);										// Sets current address at address of SEQ2_PRESET_STEP.
	*errors+=CHAN_setByte(IO24_NUM,seqC->presetStep);										// Writes value of seqB.presetStep at current address.
	errors+=CHAN_command(IO24_NUM,7);														// Executes IO24 command 0x07 named STEPCOUNT_SEQUENCER_RUN because SEQ_PRESET_STEP or SEQ2_PRESET_STEP have been edited.
	displayMainMenu();																		// Displays main menu.
}

void add10Steps(sequencer* seqA, sequencer* seqB, sequencer* seqC, uint32_t* errors) {     	// add10Steps() adds 10 steps to the three sequencers passed in parameters. 
	uint8_t countA=0,countB=0,countC;												 
	...
	*errors+=CHAN_addr(IO24_NUM,seqC->presetCountAddr);										// Sets current address at address of SEQ2_PRESET_COUNT.
	CHAN_getBytes(IO24_NUM,1,&countC);														// Stores value of SEQ2_PRESET_COUNT in variable countC.
	...
	*errors+=CHAN_addr(IO24_NUM,seqC->presetCountAddr);										// Sets current address at address of SEQ2_PRESET_COUNT.
	*errors+=CHAN_setByte(IO24_NUM,seqC->presetCount+countC);								// Writes 10+countC at current address.
	
	if (countA==0||countB==0||countC==0) {																// If one of the two sequencers has finished its sequence.
...
void displayStatus(sequencer* seqA, sequencer* seqB, sequencer* seqC, uint32_t* errors) { 	// displayStatus() displays sense and number of remaining steps for each sequencer. 
	uint8_t countA=0,countB=0,countC=0;																
	uint8_t stepA=0, stepB=0,stepC=0;
	*errors+=CHAN_addr(IO24_NUM,seqA->presetCountAddr);										// Sets current address at address of SEQ_PRESET_COUNT.			
	CHAN_getBytes(IO24_NUM,1,&countA);														// Stores value of SEQ_PRESET_COUNT in variable countA.
	*errors+=CHAN_addr(IO24_NUM,seqB->presetCountAddr);										// Sets current address at address of SEQ1_PRESET_COUNT.
	CHAN_getBytes(IO24_NUM,1,&countB);														// Stores value of SEQ1_PRESET_COUNT in variable countB.
	*errors+=CHAN_addr(IO24_NUM,seqC->presetCountAddr);										// Sets current address at address of SEQ2_PRESET_COUNT.
	...
	printf("Remaining step(s) for sequencer %s : %d\n",seqA->name,countA);					//
	printf("Remaining step(s) for sequencer %s : %d\n",seqB->name,countB);					// 
	printf("Remaining step(s) for sequencer %s : %d\n",seqC->name,countC);					// Displays remaining steps for the three sequencers.
	*errors+=CHAN_addr(IO24_NUM,seqA->presetStepAddr);										// Sets current address at address of SEQ_PRESET_STEP.
	...
	*errors+=CHAN_addr(IO24_NUM,seqC->presetStepAddr);										// Sets current address at address of SEQ2_PRESET_STEP.
	CHAN_getBytes(IO24_NUM,1,&stepC);														// Stores value of SEQ2_PRESET_STEP in variable stepC.
	...
	if (stepC==0) printf("sequencer %s does nothing\n",seqC->name);							//
	else if (stepC==1) printf("sequencer %s increases\n",seqC->name);						//
	else if (stepC==2) printf("sequencer %s decreases\n",seqC->name);						// Displays if Seq1 does nothing, increases or decreases.
	displayMainMenu();																		// Displays main menu.		
}
...
int main (int argc,char *argv[ ]) {
	...
	************************************************************************************************************************************************************************************************************************************************/
	...
	myTXBuffer[7]=3;								// SEQ_MODE=3 : 		 the two sequencers Seq and Seq2 are independent and play a common sequence asynchronously. Seq and Seq2 settings are required.
	...
	************************************************************************************************************************************************************************************************************************************************/
	...
	printf("\nIO24 stepcount sequenced outputs mode 3 is started : \n");
	...
	while (!exit_loop) {														// Entering loop
		...
		if (c) {                                                    			//  
			...
			else if (c=='w') invertWrap(&seqs,&errors);							//	 Else if 'w' is typed inverts wrap of the three sequencers (even if Seq1 is unused). 
			else if (c=='p') pauseResumeSteps(&seq,&seq1,&seq2,&errors);				//	 Else if 'p' is typed pauses/resumes the two sequencers Seq and Seq2. 
			else if (c=='a') add10Steps(&seq,&seq1,&seq2,&errors);					//	 Else if 'a' is typed adds 10 steps to two sequencers Seq and Seq2. 
			else if (c=='?') displayStatus(&seq,&seq1,&seq2,&errors);			//   Else if '?' is types displays status.
			else if (c=='0') exit_loop=displaySeqMenu(c,&seq,&errors); 			// 	 Else if '0' is typed displays Seq menu.
			else if (c=='1') exit_loop=displaySeqMenu(c,&seq1,&errors); 			// 	 Else if '0' is typed displays Seq menu.															
