 /*******************************************************************
 *                                                                  * 
 * Chantilly 0x10 - IO24 : Source code for application note AN111   *
 *                                                     			    *
 ****************************************************************************************************************************************************************************************************************************************************
 * CIRCUIT DESCRIPTION :																																					                                                                        *
 ...
 * As described in file chantilly_0x10_AN111_pcb2rev1.PDF, a bar led is wired on OutputA and OutputB.																																				*
 * 																																																													*
 *  																																																												* 
 * PROGRAM DESCRIPTION : Test using IO24 direct core access mode with (immediate inputs and) sequenced outputs in mode 1.                                                                    		                                               	*
 ...
 * SEQ_MASK, SEQ1_MASk initial values make Seq outputted on OutputA bit0, bit1, bit2 and bit3 and on all bits of OutputB, Seq1 outputted on bit4, bit5, bit6  and bit7 of outputA.																	*
 *																																																													*
 *  												                                                   																																				*		
 *                                       									                  													                                                   													*
...
{																		// SEQ_WRAP is a common setting to the sequencers Seq and Seq1 in mode1. 
	uint8_t wrap;														
	uint32_t wrapAddr;
}
sequencers;                                                             
...
void initCursor(sequencer* seqN,uint32_t* errors) {						// initCursor() initializes cursor of the sequencer passed in parameter.
	if (seqN->step==1) seqN->cursor=0x04;								// Using mode 1 implies Seq or Seq1 can be passed in parameter.
...
uint32_t displaySeqMenu(char c,sequencer* seqN,uint8_t* exit_loop) { 						// displaySeqMenu() lets the user edit specific settings of the sequencer passed in parameterr.
	uint8_t c2='\0';																		// Using mode 1 implies Seq or Seq1 can be passed in parameter.
	uint32_t errors=0;
	uint8_t buf[8];
		...
		if (c2=='q') *exit_loop=1;															// If character typed is 'q' : Exits loop.
			...
			if (c2=='m') {																	// If character typed is 'm' : Edits the mask.
				...
				if (c!='0') { 																//  If the sequencer passed to the function is not Seq.
					errors+=CHAN_addr4(IO24_NUM,seqN->stepAddr4);							//   Sets current address at address of SEQ1_STEP into IO24.
				...
				if (c!='0') {																//  If the sequencer passed to the function is not Seq.
					errors+=CHAN_setByte(IO24_NUM,2);										//   Writes 2 at current address to define Seq1 as sequencer to freeze.
					...
					printf("Enter new value for SEQ%c_ASTEP_INC_TIME=",c);					//   Asks a new value for SEQ1_ASTEP_INC_TIME.
					scanf("%3d",(unsigned int*)&seqN->astepIncTime);						//   Stores the new value of SEQ1_ASTEP_INC_TIME into variable seqN.astepIncTime.
					CHAN_command(IO24_NUM,2);												//   Executes IO24 command 0x02 named FREEZE. Freezes the sequencers defined by SEQS_FREEZE_MASK.
					errors+=CHAN_addr(IO24_NUM,seqN->astepIncTimeAddr);						//   Sets current address at address of SEQ1_ASTEP_INC_TIME.
					...
					printf("(SEQ%c_ASTEP_INC_TIME+1)=%dms\n",c,(seqN->astepIncTime+1)*5);	//   Displays new steps delay of Seq1.
				}                                   
				else {																		//  Else
				...
				errors+=CHAN_setByte(IO24_NUM,0);											//    Writes 0 at current address : To synchronize Seq1 with any step of Seq.
				...
				errors+=CHAN_addr(IO24_NUM,0x1CB);											//  Sets current address at address of SEQS_FREEZE_MASK.
				if (c!='0') {																//  If the sequencer passed to the function is not Seq.
					if (!frozenA) {															//   If Seq1 is not frozen
						errors+=CHAN_setByte(IO24_NUM,2);									//    Writes 2 at current address to define Seq1 as sequencer to freeze.
						CHAN_command(IO24_NUM,2);											//    Executes IO24 command 0x02 named FREEZE : The sequencers defined by SEQS_FREEZE_MASK are frozen.
						frozenA=1;
						printf("Sequencer Seq1 is frozen.\n");
						fflush(stdout);
					}
					else {																	//   Else
						errors+=CHAN_setByte(IO24_NUM,2);									//    Writes 2 at current address to define Seq1 as sequencer to release.
						errors+=CHAN_addr(IO24_NUM,0x14B);									//    Sets current address at address of SEQS_UNFREEZE_POSITION into IO24.
						errors+=CHAN_setByte(IO24_NUM,0);									//    Writes 0 at current address : To synchronize Seq1 with any step of Seq.
						CHAN_command(IO24_NUM,4);											//    Executes IO24 command 0x04 named RELEASE : The sequencers defined by SEQS_FREEZE_MASK are released and synchronized.
						frozenA=0;
						printf("Sequencer Seq1 is released.\n");							//
						errors+=CHAN_addr(IO24_NUM,0x1C9);									//
						errors+=CHAN_getBytes(IO24_NUM,1,buf);								//
						printf("It has been frozen during\tTf=SEQ1_FREEZE_COUNT*Ts1");		//
						printf(" with SEQ1_FREEZE_COUNT=%d\n",buf[0]); 						//
						printf("\t\t\t\t");													//
						printf("Tf=SEQ1_FREEZE_COUNT*(SEQ1_ASTEP_INC_TIME+1)*Tseqs\n");		//
						printf("\t\t\t\t");													//
						printf("Tf=%d*(%d+1)*5ms\n",buf[0],seqN->astepIncTime);				//
						printf("\t\t\t\t");													//
						printf("Tf=%dms\n",buf[0]*(seqN->astepIncTime+1)*5);				//    Displays SEQ1_FREEZE_COUNT and freeze time values.
						fflush(stdout);														//
					}
				}
				else {																		//  Else
					if (!frozenB) {															//   If Seq is not frozen.
						...
						errors+=CHAN_addr(IO24_NUM,0x14B);									//    Sets current address at address of SEQS_UNFREEZE_POSITION into IO24.
						errors+=CHAN_setByte(IO24_NUM,0);									//    Writes 0 at current address : To synchronize Seq with any step of Seq1.	 
						CHAN_command(IO24_NUM,4);											//    Executes IO24 command 0x04 named RELEASE : The sequencers defined by SEQS_FREEZE_MASK are released and synchronized.
						frozenB=0;
				...
				if (c!='0') {																//  If the sequencer passed to the function is not Seq.
					if (!frozenA) {															//   If Seq1 is not frozen
						errors+=CHAN_setByte(IO24_NUM,2);									//    Writes 2 at current address to define Seq1 as sequencer to freeze.
						CHAN_command(IO24_NUM,2);											//    Executes IO24 command 0x02 named FREEZE : The sequencers defined by SEQS_FREEZE_MASK are frozen.
						errors+=CHAN_addr(IO24_NUM,0x01);									//    Sets current address at address of SEQ1_SAVED_CURSOR.
						errors+=CHAN_setByte(IO24_NUM,seqN->mask);							//    Writes seqN.mask at current address to force all bits defined by SEQ1_MASK to 1 on OutputA.
						frozenA=1;
						printf("Sequencer Seq1 is frozen.\n");
						fflush(stdout);
					}
					...
					if (!frozenB) {															//   If Seq is not frozen.
						...
						errors+=CHAN_setByte(IO24_NUM,seqN->mask);							//    Writes seqN.mask at current address to force all bits defined by SEQ_MASK to 1 on OutputA.
						errors+=CHAN_addr(IO24_NUM,0x03);									//    Sets current address at address of SEQ_P128_SAVED_CURSOR.
						errors+=CHAN_setByte(IO24_NUM,0xF0);								//    Writes 0xF0 at current address to force bits 7, 6, 5 and 4 to 1 on OutputB (In mode 1 OutputB is not masked).
						frozenB=1;
						printf("Sequencer Seq is frozen.\n");
						fflush(stdout);
					}
				...
				errors+=CHAN_addr(IO24_NUM,0x1CB);											//  Sets current address at address of SEQS_FREEZE_MASK.
				if (c!='0') {																//  If the sequencer passed to the function is not Seq.
					errors+=CHAN_setByte(IO24_NUM,2);										//   Writes 2 at current address to define Seq1 as sequencer to freeze and resync.
					errors+=CHAN_addr(IO24_NUM,0x14B);										//   Sets current address at address of SEQS_UNFREEZE_POSITION into IO24.
					errors+=CHAN_setByte(IO24_NUM,4);										//   Writes 4 at current address : To synchronize Seq1 with the first step of Seq (when SEQ_CURSOR=4). 
					if (seqN->step==1) seqN->cursor=4;										//   If Seq1 increases presets its cursor at the start of the sequence.
					else if (seqN->step==2) seqN->cursor=7;									//   If Seq1 decreases presets its cursor at the end of the sequence.
					CHAN_command(IO24_NUM,2);												//   Executes IO24 command 0x02 named FREEZE. Freezes the sequencers defined by SEQS_FREEZE_MASK.
					errors+=CHAN_addr(IO24_NUM,seqN->cursorAddr);							//   Sets current address at address of SEQ1_CURSOR.
					errors+=CHAN_setByte(IO24_NUM,seqN->cursor);							//   Writes new value of SEQ1_CURSOR
					CHAN_command(IO24_NUM,4);												//   Executes IO24 command 0x04 named RELEASE : The sequencers defined by SEQS_FREEZE_MASK are released and synchronized.
					printf("Sequencer Seq1 is synchronised with sequencer Seq as reference.\n");
					fflush(stdout);
				}
				else {
					errors+=CHAN_setByte(IO24_NUM,1);										//   Writes 1 at current address to define Seq as sequencer to freeze and resync.
					errors+=CHAN_addr(IO24_NUM,0x14B);										//   Sets current address at address of SEQS_UNFREEZE_POSITION into IO24.
					errors+=CHAN_setByte(IO24_NUM,4);										//   Writes 4 at current address : To synchronize Seq with the first step of Seq1 (when SEQ1_CURSOR=4).
					...
					CHAN_command(IO24_NUM,4);												//   Executes IO24 command 0x04 named RELEASE : The sequencers defined by SEQS_FREEZE_MASK are released and synchronized.
					printf("Sequencer Seq is synchronised with sequencer Seq1 as reference.\n");
					fflush(stdout);
				}
			}
...
int main (int argc,char *argv[ ]) {
	...
	sequencer seq={0x0F,0x150,1,0x50,0x04,0x141,29,0x144,1,0x145};	// The sructure storing specific settings of sequencer Seq.
	//seq.mask=0x0F;								 
	//seq.maskAddr=0x150;							 
	//seq.step=1;									  									
	//seq.stepAddr4=0x50;														
	...
	//seq.unfreezeResync=1;
	//seq.unfreezeResyncAddr=0x145;		
	
	sequencer seq1={0xF0,0x151,1,0x70,0x04,0x1C1,29,0x1C2,0,0x1C3};	// The sructure storing specific settings of sequencer Seq1.
	//seq1.mask=0xF0;										
	//seq1.maskAddr=0x151;
	//seq1.step=2;									
	//seq1.stepAddr4=0x70;							
	...
	//seq1.unfreezeResync=0;
	//seq1.unfreezeResyncAddr=0x1C3;		
	
	sequencers seqs={1,0x146};										// The structure storing common settings of the enforced sequencers Seq and Seq1.
	...
	************************************************************************************************************************************************************************************************************************************************/
	...
	// Sequencer mode is now set with update period, Tseqs, such as Tseqs=(0xFFFF-SEQ_USER_VAL)*100ns/PRESCAseq=12535*100ns*4=5.014ms.
	// That means, inputs are read every Tseqs=5.014ms and sequenced outputs are updated every Ts with Ts=(SEQ_ASTEP_INC_TIME+1)*Tseqs=(SEQ_ASTEP_INC_TIME+1)*5.014ms. 
	...
	************************************************************************************************************************************************************************************************************************************************/
	...
	myTXBuffer[0]=seq.step;   						// SEQ_STEP=1 : 		   sequencer Seq increases.
	...
	myTXBuffer[2]=0x04;								// SEQ_START=0x04 : 	   RAM address where (common) Seq and Seq1 sequence begins.
	myTXBuffer[3]=0x07;   							// SEQ_END=0x08 : 		   RAM address where (common) Seq and Seq1 sequence ends. In mode 1, 0x04≤SEQ_START≤SEQ_END≤0x7F.
	myTXBuffer[4]=seq.astepIncTime;					// SEQ_ASTEP_INC_TIME :    used to change delay Ts between steps of Seq sequence such as Ts=(SEQ_ASTEP_INC_TIME+1)*Tseqs=31*0.005014=155.434 ms. => fs=6.43Hz. 
	myTXBuffer[5]=seq.unfreezeResync;				// SEQ_UNFREEZE_RESYNC=1 : Seq is synchronized with Seq1 when released.
	...
	myTXBuffer[7]=1;								// SEQ_MODE=1 : 		   the two sequencers Seq and Seq1 are independent and play a common sequence asynchronously. That means sequencers Seq and Seq1 settings are required.
	...
	/** SETS UP SEQUENCER Seq1 SETTINGS */
													// Presets via myTXBuffer :
	myTXBuffer[0]=seq1.step;   						// SEQ1_STEP=1 : 			sequencer Seq1 increases.
	...
	myTXBuffer[2]=seq1.astepIncTime;				// SEQ1_ASTEP_INC_TIME :    used to change delay Ts1 between steps of Seq1 sequence such as Ts1=(SEQ1_ASTEP_INC_TIME+1)*Tseqs=31*5.014ms=155.434 ms. => fs1=6.43Hz. 
	myTXBuffer[3]=seq1.unfreezeResync;				// SEQ1_UNFREEZE_RESYNC=0 : Seq1 is synchronized with Seq when released.
	...
	************************************************************************************************************************************************************************************************************************************************/
	...
	printf("\nIO24 sequenced outputs mode 1 is started : \n");
	printf("Sequencers Seq and Seq1 are running with respective steps delays Ts and Ts1 set such as ");
	printf("Ts=Tseqs*(SEQ_ASTEP_INC_TIME+1) and Ts1=Tseqs*(SEQ1_ASTEP_INC_TIME+1) ");
	printf("with SEQ_ASTEP_INC_TIME=SEQ1_ASTEP_INC_TIME=30 and Tseqs=5ms ");
	printf("then Ts=Ts1=31*5ms=155ms\n\n");
	displayMainMenu();//printf("\tType\t:\t'0' to edit Sequencer Seq.\n\t\t\t'1' to edit Sequencer Seq1.\n\t\t\t'w' to invert wrap.\n\t\t\t'p' to pause/resume.\n\t\t\t'q' to exit.\n");
	...
	while (!exit_loop) {															// Entering loop
		...
		if (c) {                                                    				//  if something is typed :
			...
			else if (c=='p') errors+=pauseResumeSteps(&seq,&seq1);					//	 if 'p' is typed pauses/resumes the three sequencers. pauseSteps() takes &seq and &seq1 as parameters because SEQ_STEP and SEQ1_STEP are independent.
			else {														
