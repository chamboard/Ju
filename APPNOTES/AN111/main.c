	...
	if (c2) {
		if (c2=='q') *exit_loop=1;															// If character typed is 'q' : Exits loop.
			...
			if (c2=='m') {																	// If character typed is 'm' : Edits the mask.
				...
				errors+=CHAN_addr(IO24_NUM,seqN->maskAddr);									//  Switch the parameter seqN passed to the function, sets current address at address of SEQ_MASK or SEQ1_MASK into IO24.
				...
				errors+=CHAN_addr4(IO24_NUM,seqN->stepAddr4);								//   Switch the parameter seqN passed to the function, sets current address at address of SEQ_STEP or SEQ1_STEP.
				...
				else if (seqN->step==2) printf("sequencer Seq%c decreases\n",c);			//   Switch the parameter seqN passed to the function, displays the new sense of the sequencer Seq or Seq1. 
			}
				...
				printf("Seq%c steps delay Ts%c is such as",c,c);							//   Displays SEQ_ASTEP_INC_TIME or SEQ1_ASTEP_INC_TIME and current steps delay of the sequencer Seq or Seq1.
				printf(" Ts%c=Tseqs*(SEQ%c_ASTEP_INC_TIME+1)",c,c);							//
				printf(" with Tseqs=5ms and ");												//
				printf("SEQ%c_ASTEP_INC_TIME=%d\n",c,seqN->astepIncTime);					//
				printf("Enter new value for SEQ%c_ASTEP_INC_TIME=",c);						//   Asks a new value for SEQ_ASTEP_INC_TIME or SEQ1_ASTEP_INC_TIME.
				...
				errors+=CHAN_addr(IO24_NUM,seqN->astepIncTimeAddr);							//   Sets current address at address of SEQ_ASTEP_INC_TIME or SEQ1_ASTEP_INC_TIME.
				...
				printf("(SEQ%c_ASTEP_INC_TIME+1)=%dms\n",c,(seqN->astepIncTime+1)*5);		//   Switch the sequencer passed to the function, displays new steps delay of Seq or Seq1.
				...
				if (!seqN->frozen){
					...
					printf("\t\t\t\t");														//
					printf("Tf%c=%dms\n",c,buf[0]*(seqN->astepIncTime+1)*5);				//    Switch the sequencer passed to the function, displays SEQ_FREEZE_COUNT or SEQ1_FREEZE_COUNT and frozen time values.
					fflush(stdout);
				}
			}
				...
				if (!seqN->frozen){
					CHAN_command(IO24_NUM,2);												//    Executes IO24 command 0x02 named FREEZE : The sequencers defined by SEQS_FREEZE_MASK are frozen.
					errors+=CHAN_addr(IO24_NUM,seqN->savedCursorAddr);						//    Sets current address at address of SEQ_SAVED_CURSOR or SEQ1_SAVED_CURSOR.
					errors+=CHAN_setByte(IO24_NUM,seqN->mask);								//    Writes seqN.mask at current address to force all bits defined by seqN.mask to 1 on OutputA.
					if (c==0) {																//    Is Seq is the sequencer passed to the function
						errors+=CHAN_addr(IO24_NUM,0x03);									//     Sets current address at address of SEQ_P_128_SAVED_CURSOR.
						errors+=CHAN_setByte(IO24_NUM,0xF0);								//     Writes seqN.mask at current address to force all bits 7, 6, 5 and 4 to 1 on OutputB.
					}																		//     (In mode 1 SEQ_P128_SAVED_CURSOR, the saved value of SEQ_CURSOR+128, must be overwritten to force OutputB.)  
					seqN->frozen=1;
					printf("Sequencer Seq%c is frozen.\n",c);								//
				}
				else {
				...
				CHAN_command(IO24_NUM,2);													//   Executes IO24 command 0x02 named FREEZE. Freezes the sequencers defined by SEQS_FREEZE_MASK.
				errors+=CHAN_addr(IO24_NUM,seqN->cursorAddr);								//   Sets current address at address of SEQ_CURSOR or SEQ1_CURSOR.
				errors+=CHAN_setByte(IO24_NUM,seqN->cursor);								//   Writes new value of SEQ_CURSOR or SEQ1_CURSOR switch parameter of the function.
				CHAN_command(IO24_NUM,4);													//   Executes IO24 command 0x04 named RELEASE : The sequencers defined by SEQS_FREEZE_MASK are released and synchronized.
...
int main (int argc,char *argv[ ]) {
	...
	sequencer seq={0x0F,0x150,1,0x50,0x04,0x141,29,0x144,1,0x145,0,0x00,0,0x1C8,0,1};	// The sructure storing specific settings of sequencer Seq.
	//seq.mask=0x0F;								 
	//seq.maskAddr=0x150;							 
	//seq.step=1;									  									
	//seq.stepAddr4=0x50;														
	...
	sequencer seq1={0xF0,0x151,2,0x70,0x07,0x1C1,29,0x1C2,0,0x1C3,0,0x01,0,0x1C9,0,2};	// The sructure storing specific settings of sequencer Seq1.
	//seq1.mask=0xF0;										
	//seq1.maskAddr=0x151;
	//seq1.step=2;									
	//seq1.stepAddr4=0x70;							
	...
	************************************************************************************************************************************************************************************************************************************************/
	...
	myTXBuffer[00]=0x88;							// Seq sequence on OutputB			
	myTXBuffer[01]=0x44;							// (2nd sequence of the couple)	
	myTXBuffer[02]=0x22;							//  							
	myTXBuffer[03]=0x11;							//								
	...
	myTXBuffer[5]=seq.unfreezeResync;				// SEQ_UNFREEZE_RESYNC=1   	Seq is synchronized with Seq1 when released : Seq1 is time reference for Seq.
	...
	myTXBuffer[3]=seq1.unfreezeResync;				// SEQ1_UNFREEZE_RESYNC=0   Seq1 is synchronized with Seq when released : Seq is time reference for Seq1.
	...
	************************************************************************************************************************************************************************************************************************************************/
	...
	printf("with SEQ_ASTEP_INC_TIME=SEQ1_ASTEP_INC_TIME=29 and Tseqs=5ms ");
	printf("then Ts=Ts1=30*5ms=150ms\n\n");
